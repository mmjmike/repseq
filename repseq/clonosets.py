...
def find_all_mixcr_clonosets(folders, chain=None, show_offtarget=True, offtarget_threshold=0.01):
    ...
    Returns:
        pd.DataFrame: table with `sample_id`, `chain`, and `filename` columns.
            If filenames include a `mix_id` prefix, such as
            `mix_id.sample_id.clones_TRA.tsv`, the table also includes a
            `mix_id` column.
...
def find_all_exported_clonosets_in_folder(folder, chain=None, show_offtarget=True, offtarget_threshold=0.01):
    ...
    for f in all_files:
        ...
        mix_id = None
        if "." in sample_id:
            mix_id, sample_id = sample_id.split(".", 1)
...
#     if colnames is None:
#         colnames = get_column_names_from_clonoset(clonoset)
#     clonoset = clonoset.loc[~clonoset[colnames["cdr3aa_column"]].str.contains("\*|_", na=False)]
